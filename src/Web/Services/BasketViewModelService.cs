...
namespace Web.Services
{
    public class BasketViewModelService : IBasketViewModelService
    {
        ...
        public async Task<BasketViewModel> AddBasketItemAsync(int productId, int quantity)
        {
            string buyerId = GetOrCreateBuyerId();
            Basket basket = await GetOrCreateBasketAsync(buyerId);
        ...
        public async Task UpdateBasketItemsAsync(int[] basketItemIds, int[] quantities)
        {
            ...
            if (basketItemIds.Length != quantities.Length)
                throw new ArgumentException("Basket item ids and quantities do not match.");
        ...
        public async Task<Order> CreateOrderAsync(Address address)
        {
            ...
            {
                var productIdOfOrderItem = item.ProductId;
                var productQuantityOfOrderItem = item.Quantity;
                var product = await _productRepository.GetByIdAsync(productIdOfOrderItem);
                product.SalesQuantity += productQuantityOfOrderItem;
                product.StockQuantity -= productQuantityOfOrderItem;
